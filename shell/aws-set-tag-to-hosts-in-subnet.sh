...
read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  ...
  -v --value [arg] Value to be set to the tag
  -h --help        This page
EOF
...
[ -z "${arg_v:-}" ]     && help      "Value for tag to be used -v or --value"
[ -z "${arg_s:-}" ]     && help      "Subnet should be provided using -s or --subnet"

### Runtime
##############################################################################

subnet_id=$(aws ec2 describe-subnets --filter Name="cidr",Values=${arg_s} --profile ${arg_p} | jq '.Subnets[0].SubnetId')
instance_ids=$(aws ec2 describe-instances --filters "Name=subnet-id,Values=${subnet_id}" --profile ${arg_p} | jq -r '.Reservations[].Instances[0].InstanceId')

for instance_id in ${instance_ids}
do
    aws ec2 create-tags --resources ${instance_id} --tags Key=${arg_t},Value=${arg_v} --profile ${arg_p}
done
